        ...
        <nav className="w-full z-30 top-0 pb-20 lg:pb-8 pt-9 px-4 lg:px-8">
            <div className="max-w-7xl mx-auto flex flex-wrap items-center justify-between">
                ...
                <div className="flex md:order-2 space-x-3 md:space-x-0 rtl:space-x-reverse">
                    <button type="button"  className="border-2 rounded-br-xl border-[#BF4E30] text-[#BF4E30] font-medium text-sm px-4 py-2 text-center flex items-center gap-x-1 2xl:py-2 2xl:text-xl">
                ...
                <div className={`${menuOpen ? 'flex' : 'hidden'} fixed top-0 right-0 h-full w-72 z-40 flex-col bg-black/90 md:bg-zinc-800/60 md:rounded-[30px] outline outline-1 outline-offset-[-1px] outline-neutral-700/90 overflow-hidden py-2 px-4 transition-transform duration-300 ease-in-out md:static md:flex md:flex-row md:h-auto md:w-auto md:order-1`} id="navbar-sticky">
                <div className={`${menuOpen ? 'flex' : 'hidden'} w-full justify-end mb-4`}>
                {menuOpen && <Icon icon="ri:close-fill" width="32" height="32" className="text-white my-5 cursor-pointer" onClick={() => setMenuOpen(false)}/>}
                </div>
